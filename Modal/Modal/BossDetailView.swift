...
struct BossDetailView: View {
  ...
  var body: some View {
    ...
    // Button that opens a confirmation modal to close the current view
    .overlay(
      HStack {
        Spacer()
        VStack {
    ...
    // Modal to confirm exiting or staying on the current view
    .alert("Attention!", isPresented: $showAlert, actions: {
      ...
      Text("Are you sure you want to close the current page?")
    })
